...
class Maze {
 public:
  ...
  // Read the maze from the input stream.
 ...
 private:
  ...
  Cell *At(int row, int col) {
    ...
    return &cells_[row * ncols_ + col];
  }

  bool IsVisitable(const Cell *cell) {
    return (cell->dist == -1) && isspace(cell->orig);
  }

  bool IsVisitable(int row, int col) { return IsVisitable(At(row, col)); }
  ...
  // Explore the maze; return true if a solution was found.
...
void Maze::Print() {
  ...
  const size_t iterations =
      static_cast<size_t>(nrows_) * static_cast<size_t>(ncols_);
...
void Maze::TravelBorder(int *row, int *col) {
  if (*row == 0) {
    if (*col < ncols_ - 1) {
...
Cell *Maze::Neighbor(Cell *orig, Direction dir) {
...
bool Maze::Explore(std::list<Cell *> *cells) {
  if (cells->empty()) {
    return false;
  }
